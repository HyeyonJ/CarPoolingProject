package project.carPooling.global.filter;
...
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if(!PatternMatchUtils.simpleMatch(whiteList, uri)) {
			...
			if((uri.contains("driver")) && (session == null || session.getAttribute(SessionVar.LOGIN_DRIVER) == null)) {
				//Login 안 되어 있음 > 걸러야하는 케이스
				log.info("로그인 없이 접근 시도 driver {}", uri);
			...
			if((uri.contains("passenger")) && (session == null || session.getAttribute(SessionVar.LOGIN_PASSENGER) == null)) {
				System.out.println("여기들어옴");
				//Login 안 되어 있음 > 걸러야하는 케이스
				log.info("로그인 없이 접근 시도 passenger {}", uri);
				//로그인 후 원래 이동하려고했던 경로로 이동시키기
				resp.sendRedirect("/passenger/login?redirectURL=" + uri);
				//redirect로 보낸 후 종료 > return;
				return;
			}
		}
