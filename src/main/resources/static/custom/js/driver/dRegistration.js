...
$("#datepicker").change(() => {
  ...
  if ((year + month + date === drYear + drMonth + drDate) && selectStartTimeVal !== "선택") {
...
$("#selectStartTime").change(() => {
  ...
  const selectStartTime = ($("#selectStartTime").val()).substring(0, 2) + ($("#selectStartTime").val()).substring(3, 5);
  const selectEndTime = ($("#selectEndTime").val()).substring(0, 2) + ($("#selectEndTime").val()).substring(3, 5);
...
$('#rgsBtn').click(function () {
  ...
  if ($('#selectStartTime').val() == "선택" || $('#selectEndTime').val() == "선택") {
  ...
  $.ajax({
    ...
    success: function (data) {
      console.log(data);
      if (data === 'insert') {
        swal("등록성공!", "안전운행해주시기 바랍니다.", "success").then((OK) => {
          if (OK) {
            window.location.reload();
          }
        });
      } else if (data === "출근") {
        swal("등록실패!", "해당일자에 이미 출근카풀등록내역이 있습니다. 카풀등록은 하루 총 2회 출근 1회, 퇴근 1회만 가능합니다.", "warning");
      } else if (data === "퇴근") {
        swal("등록실패!", "해당일자에 이미 퇴근카풀등록내역이 있습니다. 카풀등록은 하루 총 2회 출근 1회, 퇴근 1회만 가능합니다.", "warning");
      } else {
