...
function acceptedRsv() {
  ...
  $.ajax({
    url: '/passenger/passengerCarpool/reservation/acceptedList',
    type: 'GET',
...
function waitingRsv() {
  ...
  $.ajax({
    url: '/passenger/passengerCarpool/reservation/waitingList',
    type: 'GET',
    success: function (data) {
      console.log(data);
      var html = '';
      for (var i = 0; i < data.length; i++) {
        ...
        html += '<span id="frdate">예약일자 ' + rDate + '</span><br>\n';
        html += '<button id="view" onclick="viewRoute(' + data[i].D_STARTLON + ', ' + data[i].D_STARTLAT + ', ' + data[i].D_ENDLON + ', ' + data[i].D_ENDLAT + ')" class="btn btn-primary rsvsbtn" data-toggle="modal" data-target="#viewModal">경로보기</button>\t';
        html += '<button class="btn btn-primary rsvsbtn" id="delete" onclick="deleteReq(' + data[i].DR_IDX + ')" >예약취소</button>\t';
        // html += '<button id="delete" onclick="deleteRsv(' + P_IDX + ',' + data[i].R_IDX + ')" class="btn btn-primary rsvsbtn">카풀취소</button>';
        html += '</div>'
      }
...
/*거절*/
function deleteReq(dr_idx) {
  confirm('예약을 취소하시겠습니까?');

  $.ajax({
    url: '/passenger/passengerCarpool/reservation/delete',
    type: 'DELETE',
    data: { drIdx: dr_idx },
    success: function (data) {
      if (data != null) {
...
function refusedRsv() {
  ...
  $('#acceptedList').css('display', 'none');
  $('#pastList').css('display', 'none');
  $('#refusedList').css('display', 'block');
  $.ajax({
    url: '/passenger/passengerCarpool/reservation/refusedList',
    type: 'GET',
    success: function (data) {
      console.log(data);
      var html = '';
...
function pastRsv() {
  ...
  $.ajax({
    url: '/passenger/passengerCarpool/reservation/pastList',
    type: 'GET',
    success: function (data) {
      console.log(data);
      var html = '';
